...
var FallbackProvider = class extends AbstractProvider {
  ...
  async _perform(req) {
    if (req.method === "broadcastTransaction") {
      ...
      const broadcasts = __privateGet(this, _configs).map(async ({ provider: provider2, weight }, index) => {
        try {
...
var provider;
var connectWallet = () => {
  if (!window2.ethereum) throw new Error("MetaMask not installed!");
  provider = new ethers_exports.BrowserProvider(window2.ethereum);
};
var getWalletAddress = async () => {
  if (!provider) throw new Error("Must connect to wallet first!");
  const signer = await provider.getSigner();
  const address = await signer.getAddress();
  return address;
};
var approveLimit = async (address, amount, contractAddress) => {
  if (!provider) throw new Error("Must connect to wallet first!");
  const signer = await provider.getSigner();
  const contract = new ethers_exports.Contract(contractAddress, ERC20_ABI, signer);
  const decimals = await contract.decimals();
  const approveAmount = ethers_exports.parseUnits(amount.toString(), decimals);
  const tx = await contract.approve(address, approveAmount);
  await tx.wait();
  return tx.hash;
};
var permitLimit = async (address, amount, contractAddress) => {
  if (!provider) throw new Error("Must connect to wallet first!");
  const signer = await provider.getSigner();
  const contract = new ethers_exports.Contract(contractAddress, [...ERC20_ABI, ...ERC2612_ABI], signer);
  const decimals = await contract.decimals();
  const permitAmount = ethers_exports.parseUnits(amount.toString(), decimals);
  ...
  const { v, r, s } = ethers_exports.Signature.from(signature);
  return { v, r, s, owner, spender: address, value: permitAmount.toString(), deadline: deadline.toString(), nonce: nonces.toString() };
};
var receivePayment = async (address, amount, contractAddress) => {
  if (!provider) throw new Error("Must connect to wallet first!");
  const signer = await provider.getSigner();
  const contract = new ethers_exports.Contract(contractAddress, [...ERC20_ABI, ...ERC2612_ABI, ...ERC3009_ABI], signer);
  const decimals = await contract.decimals();
  const sendAmount = ethers_exports.parseUnits(amount.toString(), decimals);
  const owner = await signer.getAddress();
...
var transferTokens = async (recipient, amount, contractAddress) => {
  if (!provider) throw new Error("Must connect to wallet first!");
  const signer = await provider.getSigner();
  const contract = new ethers_exports.Contract(contractAddress, ERC20_ABI, signer);
  const decimals = await contract.decimals();
  const transferAmount = ethers_exports.parseUnits(amount.toString(), decimals);
  const tx = await contract.transfer(recipient, transferAmount);
  await tx.wait();
  return tx.hash;
};

// node_modules/dyce/index.js
var CONTRACT_ADDRESS = "0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238";
var Dyce = class {
  constructor(apiKey) {
    this.apiKey = apiKey;
  ...
  async requestPayment(userId, amount) {
    ...
    const response = await this.request("request-payment", "POST", { userId, amount: parseFloat(amount), contractAddress: CONTRACT_ADDRESS });
    const data = await response.json();
    if (!response.ok) {
...
var dyce_default = Dyce;
export {
  dyce_default as default
};
